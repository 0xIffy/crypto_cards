...
const CandyMachine = ({ walletAddress }) => {
  ...
  const getCandyMachineState = async () => {
    ...
    if(data.length !== 0){
      for(let mint of data){
        const response = await fetch(mint.data.uri);
        const parse = await response.json();
        console.log("Past Minted NFT", mint);

        if(!mints.find((m) => m.img === parse.image)) {
          setMints((prevState) => [...prevState, { img: parse.image, name: mint.data.name }]);
        }
      }
    }
      ...
      <div className="gif-grid">
        {mints.map((mint) => (
          <div className="gif-item" key={mint}>
            <img src={mint.img} alt={`Minted NFT ${mint}`} />
            <p>{mint.name}</p>
            {console.log(mint)}
          </div>
        ))}
  ...
  const renderDropTimer = () => {
    ...
    const dropDate = new Date(machineState.goLiveDate * 1000);
  ...
  return (
    machineState && (
      <div className="machine-container">
        {renderDropTimer()}
        {/* <p>Drop Date: {machineState.goLiveDateTimeString}</p> */}
        <p>Items Minted: {machineState.itemsRedeemed} / {machineState.itemsAvailable}</p>
        {machineState.itemsAvailable === machineState.itemsRedeemed ? (
          <p className="sub-text">Sold Out</p>
        ) : (
          <button className="cta-button mint-button" onClick={mintToken} disabled={isMinting || machineState.goLiveDate * 1000 > new Date().getTime()}>
            Mint NFT
          </button>
        )}
